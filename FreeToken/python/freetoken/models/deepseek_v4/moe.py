...
from freetoken.pruning.runtime import active_capture, apply_expert_mask
...
class Gate(BaseOP):
    ...
    def forward(self, x: torch.Tensor, input_ids: torch.Tensor):
        ...
        if self.hash:
            ...
            scores = apply_expert_mask(scores, self._expert_mask)
...
class MoE(BaseOP):
    ...
    def forward(self, x: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        if capture is not None:
            capture.begin_layer(self._layer_id, indices, weights)
