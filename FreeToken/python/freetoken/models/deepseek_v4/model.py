...
from freetoken.pruning.runtime import active_capture
...
class Block(BaseOP):
    ...
    def prefill_batched(self, x, input_ids, segments, flat_positions):
        ...
        if capture is not None:
            before = self.hc_post(torch.zeros_like(x), residual, post, comb)
            after = self.hc_post(capture.routed_output(self.layer_id).view_as(x), residual, post, comb)
            capture.finish_layer(self.layer_id, before, after, post)
        x = self.hc_post(x, residual, post, comb)
        return x
