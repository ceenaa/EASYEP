...
class Engine:
    def __init__(self, config: EngineConfig):
        ...
        from freetoken.pruning.runtime import PruningSession

        self._expert_pruning = (
            PruningSession(config) if config.expert_mask_path or config.expert_stats_path else None
        )
        ...
        if self._expert_pruning is not None:
            self._expert_pruning.attach(self.model)
    ...
    def forward_batch(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        pruning = self._expert_pruning
        if pruning is not None and pruning.path is not None and use_graph:
            raise RuntimeError("Expert statistics require eager execution")
        capture = pruning.capture(batch) if pruning is not None else nullcontext()
        with self.ctx.forward_batch(batch), self.model.forward_host_ctx(batch, use_graph), capture:
