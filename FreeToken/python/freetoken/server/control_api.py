...
def register_control_routes(
    ...
    @app.get("/v1/pruning/config")
    async def pruning_config():
        from freetoken.pruning.artifacts import server_metadata

        state = get_state()
        return {**server_metadata(state.config), "instance_id": getattr(state, "instance_id", None)}
